...
export default function Addresses() {
    ...
    const fetchAddresses = async () => {
        try {
            setAddresses(dummyAddress as Address[]);
        } catch (error) {
            Toast.show({
                type: 'error',
    ...
    const newData = {
        _id: isEditing && editingId ? editingId : Date.now().toString(),
        type,
        street,
        city,
        state,
    ...
    try {
        ...
        if (isEditing) {
            ...
            updatedAddresses = addresses.map((item) =>
                item._id === editingId ? newData : item
            );
        } else {
            // ADD
            updatedAddresses = [newData, ...addresses];
        }

        // handle default logic (only one default)
        if (newData.isDefault) {
            updatedAddresses = updatedAddresses.map((item) => ({
                ...item,
                isDefault: item._id === newData._id,
            }));
        }
    ...
    Alert.alert("Delete Address", "Are you sure?", [
        { text: "Cancel", style: "cancel" },
        {
            ...
            style: "destructive",
            onPress: () => {
                const updated = addresses.filter(item => item._id !== id);
